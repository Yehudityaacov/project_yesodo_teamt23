...
bool checkValid_password(char *password)
{
	int i = 0;
...
{
	if (strcmp(status, "single") && strcmp(status, "SINGLE") && strcmp(status, "Single") && strcmp(status, "married") && strcmp(status, "MARRIED") && strcmp(status, "Married") && strcmp(status,"divorced")&&strcmp(status,"DIVORCED") && strcmp(status,"Divorced") &&strcmp(status,"widow")&&strcmp(status,"WIDOW")&&strcmp(status,"Widow")&&strcmp(status,"widoer")&&strcmp(status,"WIDOER")&&strcmp(status,"Widoer"))
		return false;
	else
		return true;
}
...
Request *fill_form_changePersonalStatus()
{
	...
	else
	{
		...
		gets(new_status);
		if (!checkValid_personalStatus(new_status))
		{
			do
			{
				printf("your new personal status is NOT valid. please try again:\n");
				gets(new_status);

			} while (!checkValid_personalStatus(new_status));
...
Request *fill_form_newBorn()
{
	...
	else
	{
		...
		if (!checkID_database(motherID))
		{
			...
			} while (!checkID_database(motherID));
		}
		...
		gets(babyName);//// add valid check there is no digits in the name???????
...
Request *fill_form_changeLastName()
{
	...
	else
	{
		...
		if (!checkMatch_id_lastName(userID, current_lastName))
		{
			do
			{
				printf("oops! the last name you entered doesn't match the one you have in our database.\n");
				printf("please try again: \n");
				gets(current_lastName);

			} while (!checkMatch_id_lastName(userID, current_lastName));

		}

		printf("please enter new last name:\n");
		gets(new_lastName); //////check there is no digits in last name?????needed?
...
User *userRegistration()
{
	...
	SetConsoleTextAttribute(hConsole, YELLOW);
	printf("For clarifaction! your password must be at between 4 to 8 charcters and can contain only digits and letters(capitals or lowers)\n");
	SetConsoleTextAttribute(hConsole, WHITE);
	gets(password);
	
	/*if (!checkValid_password(password))
	{
		do
		{
			printf("oops! your password is not VALID! please try again:\n");
			gets(password);
		} while (!checkValid_password(password));
	}*/
